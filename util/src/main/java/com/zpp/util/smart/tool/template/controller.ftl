package com.${company!''}.<#if module??>${module}.</#if>controller<#if admin??>.${admin}</#if>;
...
public class ${model}Controller extends BaseController {
	...
	public @ResponseBody Result save(
		...
		<#list fieldList as field>
			@ApiParam(<#if field.description??>value = "${field.description}"</#if><#if field.nullableStr == "false">, required = true</#if>)<#if field.nullableStr == "false"> @ValidateParam({ Validator.NOT_BLANK })</#if><#if field.fieldType == "Date"> @DateTimeFormat(pattern = DateUtils.DATE_FULL_FORMAT) Date<#else> ${field.fieldType}</#if> ${field.fieldName}<#if field_has_next>,</#if>
		</#list>
			) {
		${model} ${_model};
		if (id == null) {
...
<#if containEnable>
	...
	public @ResponseBody Result enable(
		...
		${_model}Service.enable(isEnable, getAjaxIds(ids));
		return Result.createSuccessResult();
	}
</#if>
